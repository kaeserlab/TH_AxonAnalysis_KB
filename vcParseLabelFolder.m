function [volumeList,labelList,classIndices] = vcParseLabelFolder(dirPath,vResizeFactor,zStretch)
...
for i = 1:length(files)
    fName = files(i).name;
    if ~contains(fName,'Class') && ~contains(fName,'.mat') && fName(1) ~= '.' && ~contains(fName,'.db')
        nVolumes = nVolumes+1;
...
%     disp(volumePaths{i})
%     V = double(readDV(volumePaths{i}));
    V = double(volumeRead(volumePaths{i}))/65535;
    ...
    for j = 1:nClasses
%         classJ = readDV([imp filesep imn sprintf('_Class%d.dv',classIndices(j))]);
%         disp([imp filesep imn sprintf('_Class%d.tif',classIndices(j))])
